...
class Utm
{
    ...
    public function init() {
        ...
        $this->lat0 = 0.0;
        $this->long0 = ((6 * abs( $this->zone )) - 183) * \ShpAdapter\Proj4\Common::D2R;
        $this->x0 = 500000.0;
        ...
        $this->ml0 = $this->a * \ShpAdapter\Proj4\Common::mlfn( $this->e0, $this->e1, $this->e2, $this->e3, $this->lat0 );
    }
    ...
    public function forward( $p ) {
        ...
        if( isset($this->sphere) && $this->sphere === true ) { /* spherical form */
            ...
            $ml = $this->a * \ShpAdapter\Proj4\Common::mlfn( $this->e0, $this->e1, $this->e2, $this->e3, $lat );
    ...
    public function inverse( $p ) {
        ...
        if( isset($this->sphere) && $this->sphere === true ) { /* spherical form */
            ...
            $lat = \ShpAdapter\Proj4\Common::asinz( $con );
            if( $temp < 0 )
                $lat = -$lat;
            if( ($g == 0) && ($h == 0) ) {
                $lon = $this->long0;
            } else {
                $lon = \ShpAdapter\Proj4\Common::adjust_lon( atan2( $g, $h ) + $this->long0 );
            }
            ...
            for( $i = 0; true; $i++ ) {
                ...
                $phi += $delta_phi;
                if( abs( $delta_phi ) <= \ShpAdapter\Proj4\Common::EPSLN )
                    break;
                if( $i >= $max_iter ) {
            ...
            if( abs( $phi ) < \ShpAdapter\Proj4\Common::HALF_PI ) {
                ...
                $lon = \ShpAdapter\Proj4\Common::adjust_lon( $this->long0 + ($d * (1.0 - $ds / 6.0 * (1.0 + 2.0 * $t + $c - $ds / 20.0 * (5.0 - 2.0 * $c + 28.0 * $t - 3.0 * $cs + 8.0 * $this->ep2 + 24.0 * $ts))) / $cos_phi) );
            } else {
                $lat = \ShpAdapter\Proj4\Common::HALF_PI * \ShpAdapter\Proj4\Common::sign( $y );
                $lon = $this->long0;
            }
        }
